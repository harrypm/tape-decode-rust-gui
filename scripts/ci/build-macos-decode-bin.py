...
# Triples we may produce level builds for on macOS CI
_TRIPLES: tuple[str, ...] = ("x86_64-apple-darwin", "aarch64-apple-darwin")
...
def _resolve_tape_decode_bin() -> Path:
    ...
    candidates = [
        Path("target/aarch64-apple-darwin/release/tape-decode"),
        Path("target/x86_64-apple-darwin/release/tape-decode"),
    ...
    raise FileNotFoundError(
        "Could not find tape-decode macOS binary. Build it before running this packaging script."
    )
...
def main() -> None:
    ...
    pyi_args: list[str] = [
        ...
        # Bundle icon PNG so _resolve_icon_path / setWindowIcon can find it inside onefile for dock/taskbar
        "--add-data",
